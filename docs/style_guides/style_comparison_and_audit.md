...
| **Primary Buttons** | Royal Blue `#283593` | Royal Blue `#283593` | Mixed (wood / blue `#283593`) | Custom buttons | **Evaluate 4 Warm/Natural Color Alternatives** (Forest Moss, Gilded Brass, Crimson Wax, Amber Leather) to replace jarring Royal Blue |
| **Secondary Buttons** | Outline parchment `#f3e5ab`, hover red `#8b0000` | Light wood `#5d4037` with dark shadow `0 4px 0 #3e2723` | Wood buttons | Leather / Dark wood | Adopt Light Wood (`#5d4037` + `#3e2723` shadow) for Secondary, Outline Red for Control/Modal buttons |
| **Form Inputs** | Signature line (bottom border only) | N/A | Signature line + Paw-print custom select | N/A | Adopt Signature line inputs + Paw-print custom select list site-wide |
...
| **Typography System** | `Cinzel` / `Lato` / `Caveat Brush` | `Cinzel` / `Caveat Brush` | `Cinzel` / `Caveat Brush` / `Lato` | `Cinzel` / `Lora` / `Crimson Pro` / `Lato` | **Establish Nature/Feral Medieval Font Roles**: `Cinzel`/`Almendra` (Headings), `Caveat Brush` (Forms/Bios/Bios/Signatures), `Philosopher` (UI/Labels), `Crimson Pro` (Chat) |
